...
#pragma endregion
	...
	//カメラ
	view_.eye = { 0.0f, 0.0f, -10.0f };
	view_.target = target_.translation_;
...
#pragma region DirectX毎フレーム処理
		...
		// ----- ベクトル ----- //
		Vector3 frontV = Vector3(view_.eye.x - view_.target.x,view_.eye.y - view_.target.y,view_.eye.z - view_.target.z);

		Vector3 sideV = Vector3(boomerangWidth * frontV.z, 0, -boomerangWidth * frontV.x);
		...
		if (input->TriggerKey(DIK_W) || input->TriggerKey(DIK_S)) {
			if (input->TriggerKey(DIK_W)) { angle += XMConvertToRadians(1.0f); }
